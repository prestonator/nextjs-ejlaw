...
import PracticeAreasWithSubSlugs from "@/queries/practiceAreasWithSubSlugs.graphql";

const getPage = async (subservice) => {
  try {
    const { data } = await fetchData(SubPracticeAreaData, {
      filters: {
        slug: {
          eq: subservice,
        },
...
export async function generateStaticParams() {
  const { data } = await fetchData(PracticeAreasWithSubSlugs);
  const items = data?.mainPracticeAreas?.data ?? [];
  const pairs = [];
  for (const item of items) {
    const service = item?.attributes?.slug;
    const subs = item?.attributes?.sub_practice_areas?.data ?? [];
    for (const sub of subs) {
      const subservice = sub?.attributes?.slug;
      if (service && subservice) pairs.push({ service, subservice });
    }
  }
  return pairs;
}
