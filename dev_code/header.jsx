...
export function Header({ navMenu, logo }) {
	...
	>
		...
		<div
			className={cn(
				"fixed inset-0 z-40 flex items-center justify-center bg-white transition-all duration-500 ease-in-out md:hidden",
		...
		>
			...
			>
				<Link
					href="/"
					className="relative mb-8 h-24 w-[50vw]"
					onClick={toggleMobileMenu}
				>
					{SafeImage(logo.data, "object-contain", "calc(12.24vw + 71px)", "eager")}
				</Link>
						...
						>
							{item.children ? (
								<details className="group">
									<summary className="flex cursor-pointer items-center justify-center gap-2 font-fancy text-2xl text-black">
										{item.item}
									...
									<div className="mt-4 space-y-2">
										{item.children.map((subItem) => (
											<Link
												key={subItem.id}
												href={subItem.slug}
												className="block font-fancy text-xl text-gray-300 transition-colors hover:text-black"
												onClick={toggleMobileMenu}
											>
												{subItem.item}
											</Link>
